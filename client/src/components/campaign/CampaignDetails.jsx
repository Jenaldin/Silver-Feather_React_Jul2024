...
export default function CampaignDetails() {
   ...
   return (
      <section id="section-wrapper">
         <Link
            to={`/my-boards/${username}/campaigns`}
         ...
         >
            <div className="card-profile-section" style={{ flexGrow: "1.5" }}>
               ...
               <div className="text-card-profile-section">
                  <h6 className="details-label">Language:</h6>
                  <p className="details-item-content">
                     Sessions held in {campaignDetails.language} language
                  </p>
               </div>
               <div className="text-card-profile-section">
                  <h6 className="details-label">Party size:</h6>
                  <p className="details-item-content">
                     For {campaignDetails.partySize} players
                  </p>
               </div>
               <div className="text-card-profile-section">
                  <h6 className="details-label">Shared with players?</h6>
                  <p className="details-item-content">
                     {campaignDetails.isPublic === false
                        ? "No, not yet"
                        : "Yes, it is shared with the public"}
                  </p>
               </div>
               <div className="text-card-profile-section">
               ...
               {userId === campaignDetails?.owner?._id && (
                  <>
                     <div className="text-card-profile-section">
                        ...
                        {campaignDetails.dmNotes &&
                           campaignDetails.dmNotes.length > 0 ? (
                           campaignDetails.dmNotes.map((noteObj, index) => (
                              <div key={index}>
                                 <p className="details-item-content">
                                    From Date:{" "}
                                    {new Date(noteObj.addedDate).toLocaleDateString(
                                       "en-US",
                                       {
                                          day: "numeric",
                                          month: "long",
         ...
         <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
            ...
            >
               Confirm Deletion of "{campaignDetails.title}" Campaign
            </DialogTitle>
            <DialogContent>
               <DialogContentText
                  style={{ fontWeight: "bold", textAlign: "center" }}
